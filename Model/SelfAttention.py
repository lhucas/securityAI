...
class SelfAttention(object):
    def __init__(self, input,enc_padding_mask, input_dim=256, dim_a=200, dim_r=10):
        self._input = input
        self._input_dim = input_dim
        self._dim_a = dim_a
        self._dim_r = dim_r
        self._enc_padding_mask = enc_padding_mask
        self._build_graph()
    ...
    def _build_graph(self):
        ...
        with tf.name_scope('self-attention'):
            ...
            self._Ws2 = tf.get_variable("Ws2", shape=[self._dim_a,1],dtype=tf.float64, initializer=initializer)
            Hs_2 = tf.matmul(Hs_1, self._Ws2)
            #Hs_2 = tf.reshape(Hs_2, [-1, seq_len, self._dim_r])
            Hs_2 = tf.reshape(Hs_2,[-1,1,seq_len])
            A = tf.nn.softmax(Hs_2, name='attention')
            self._A = tf.reshape(A, shape=[-1, seq_len] )
           # self._A *= self._enc_padding_mask
